...
public:
	...
	void aiOfEnemy(Knight* knight, const SafeScene* battleRoom);
	void addKeyboardEvents();//键盘事件监听
	void update(Knight* knight, const SafeScene* battleRoom);


private:
	int gold = 0;//金钱
	void attackTheKnight(Knight* knight, float disBetweenEnemyAndKnight, const SafeScene* battleRoom);//++++
	...
	void crashAttack(Knight* knight, float disBetweenEnemyAndKnight, const SafeScene* battleRoom);
	...
	bool isAdded;  
	bool isKilled;
	bool inRoom(const SafeScene* battleRoom, Point myPos);//*************
};
